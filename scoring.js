...
export function computeExam(type,mode,raw){const results={};let tyt=0,field=0,complete=true,any=false;for(const id of idsFor(type)){const r=computeTest(raw[id],id,mode);results[id]=r;if(r.entered){any=true;if(TESTS[id].group==='TYT')tyt+=r.net;else field+=r.net;}else complete=false;}if(!any)throw Error('Kaydetmeden önce en az bir ders sonucu gir.');return{results,tytNet:tyt,fieldNet:field,totalNet:tyt+field,complete,any};}

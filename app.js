...
function renderTasks(){const list=[...tasks].sort((a,b)=>(a.dueDate||a.tarih||'').localeCompare(b.dueDate||b.tarih||''));$('content').innerHTML=`<div class="card"><h1>Ödevler</h1><p class="muted">Öğretmen, veli ve koçun verdiği çalışmalar tek yerde. Sonuçlar bütün rollerce görülebilir.</p>${canAssign()?`<details><summary><strong>+ Yeni ödev ata</strong></summary><form id="assignment-form"><div class="grid"><label>Ders<select id="task-subject">${assignmentSubjectOptions()}</select></label><label>Toplam soru<input id="task-q" type="number" min="1" step="1" required></label><label>Hedef doğru (opsiyonel)<input id="task-target" type="number" min="0" step="1"></label><label>Son tarih<input id="task-date" type="date" required></label></div><label>Not<textarea id="task-note" rows="2"></textarea></label><button class="primary">Ödev ata</button></form></details>`:''}</div><div id="task-list"></div>`;if(canAssign())$('assignment-form').onsubmit=saveTask;const listEl=$('task-list');if(!list.length){listEl.innerHTML='<div class="card">Henüz ödev atanmadı.</div>';return}for(const t of list){const legacy=!t.schemaVersion;const done=t.status==='completed'||t.status==='reviewed';const card=E('section',{className:'card'});card.innerHTML=`<div class="taskhead"><h3>${esc(assignmentSubjectName(t.ders))}${t.konu?' · '+esc(t.konu):''}</h3><span class="pill">${esc(t.status||t.durum||'Bekliyor')}</span></div><p>${t.kitap?esc(t.kitap)+' · ':''}Son tarih ${dateString(t.dueDate||t.tarih)}</p><p class="muted">Atayan: ${esc(t.assignedByRole||'Eski kayıt')} ${t.questionCount?'· '+t.questionCount+' soru':''} ${t.targetCorrect!==null&&t.targetCorrect!==undefined?'· Hedef '+t.targetCorrect+' doğru':''}</p>${t.note?`<p>${esc(t.note)}</p>`:''}${t.result?`<div class="summary">${t.result.correct} doğru · ${t.result.wrong} yanlış · ${t.result.blank} boş · ${format(t.result.net)} net</div>`:''}${legacy?'<p class="warning">Eski ödev kaydı: soru sayısı ve sonuç alanları yok. Yeni formatta ödev oluşturabilirsiniz.</p>':''}`;
...
async function saveTask(e){
 ...
 try{
  ...
  await addDoc(collection(db,'Assignments'),{
   ...
   questionCount:q,targetCorrect:target,dueDate:$('task-date').value,

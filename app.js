...
import {lastFiveWidget} from './last-five-view.js';
...
/* Son beş deneme: Öğretmen, Veli, Koç ve Admin'e özel; öğrenci ana ekranı sade kalır. */
const previousAnalysisPage = renderAnalysis;
renderAnalysis = function(){
  previousAnalysisPage();
  if(!staff())return;
  const panel=document.createElement('div');
  panel.innerHTML=lastFiveWidget(exams,TESTS,lastFiveSelectedType);
  $('content').prepend(panel);
  panel.querySelector('#last5-type').addEventListener('change',event=>{
    lastFiveSelectedType=event.target.value;
    renderAnalysis();
  });
};
let lastFiveSelectedType='TYT';

/* Diğer rollerin ana ekranında son 5 analizine hızlı erişim; öğrenciyi kalabalıklaştırma. */
const previousHomePage = renderHome;
renderHome = function(){
  previousHomePage();
  if(!staff())return;
  const section=document.createElement('section');
  section.className='card';
  section.innerHTML='<h2>📈 Son 5 Deneme</h2><p class="muted">TYT, SAY, EA, SÖZ ve DİL gelişimini ayrı ayrı görüntüle.</p><button type="button" id="open-last-five" class="primary">Son 5 Deneme Analizini Aç</button>' ;
  $('content').prepend(section);
  section.querySelector('#open-last-five').onclick=()=>{view='analysis';render();};
};

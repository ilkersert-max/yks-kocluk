...
function assignmentMetrics(t){
 const assigned=Number(t.questionCount);
 const result=t.result;
 const actual=Number(result?.attempted ?? (result&&result.correct!=null&&result.wrong!=null&&result.blank!=null?Number(result.correct)+Number(result.wrong)+Number(result.blank):NaN));
 const valid=Boolean(result)&&Number.isInteger(assigned)&&assigned>0&&Number.isInteger(actual)&&actual>=0;
 const cancelled=['cancelled','iptal'].includes(String(t.status||t.durum||'').toLowerCase());
 const legacyComplete=!valid&&!result&&!cancelled&&['completed','reviewed'].includes(String(t.status||t.durum||'').toLowerCase());
 return {assigned,actual,valid,cancelled,complete:!cancelled&&(valid?actual>=assigned:legacyComplete),over:valid&&actual>assigned};
}

...
function showTaskEdit(card,t){
 ...
 form.onsubmit=async e=>{e.preventDefault();const q=parseNumber(form.elements.q.value),target=parseNumber(form.elements.target.value);
  ...
  if(t.result&&Number(t.result.attempted)>q){notify('Yeni soru sayısı önceden çözülen sorudan küçük olamaz.',true);return;}
  ...
  if(t.result){patch.result={...t.result,remaining:q-Number(t.result.attempted||0)};if(q!==Number(t.questionCount)&&['completed','reviewed'].includes(t.status))patch.status='in_progress';}
...
function showTaskResult(card,t){const old=card.querySelector('.task-editor');if(old){old.remove();return}const editor=E('form',{className:'task-editor'});editor.innerHTML=`<h3>Ödev sonucu</h3><p>Ödev ${t.questionCount} soru; kısmi çalışma girebilirsin.</p><div class="grid"><label>Çözülen soru<input name="attempted" type="number" min="0" max="${t.questionCount}" value="${t.result?.attempted??t.questionCount}" required></label><label>Doğru<input name="correct" type="number" min="0" value="${t.result?.correct??''}" required></label><label>Yanlış<input name="wrong" type="number" min="0" value="${t.result?.wrong??''}" required></label></div><label>Çözüm tarihi<input name="resultDate" type="date" required></label><label><input type="checkbox" name="finished" style="width:auto" ${t.status==='completed'?'checked':''}> Ödevi tamamladım</label><button class="primary">Sonucu kaydet</button>`;editor.elements.resultDate.value=t.resultDate||dateISO(t.resultUpdatedAt)||new Date().toLocaleDateString('sv-SE');editor.onsubmit=async e=>{e.preventDefault();const a=parseNumber(editor.elements.attempted.value),d=parseNumber(editor.elements.correct.value),w=parseNumber(editor.elements.wrong.value);if(![a,d,w].every(Number.isInteger)||a<0||d<0||w<0||d+w>a||a>t.questionCount){notify('Ödev soru sayıları geçersiz.',true);return}const finished=editor.elements.finished.checked;if(finished&&a!==t.questionCount){notify('Ödev tamamlandı işaretlemek için soru sayısını tamamla.',true);return}try{await updateDoc(doc(db,'Assignments',t.id),{result:{attempted:a,correct:d,wrong:w,blank:a-d-w,remaining:t.questionCount-a,net:d-w/4,updatedBy:user.uid},status:finished?'completed':'in_progress',resultDate:editor.elements.resultDate.value,resultUpdatedAt:serverTimestamp()});notify('Ödev sonucu kaydedildi.')}catch(err){notify(err.message,true)}};card.append(editor)}
...
function weekRows(start,end){
 ...
 for(const t of tests){const date=weeklyDate(t.studyDate||t.tarih);if(date&&date>=start&&date<=end){const d=Number(t.dogru),w=Number(t.yanlis),b=Number(t.bos);add(t.ders,d+w+b,d,w,b);}}
 for(const t of tasks){if(!t.result)continue;const date=weeklyDate(t.resultDate||t.resultUpdatedAt);if(date&&date>=start&&date<=end){const d=Number(t.result.correct),w=Number(t.result.wrong),b=Number(t.result.blank);add(t.ders,d+w+b,d,w,b);}}

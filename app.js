...
function assignmentStatus(t){
 const state=String(t.status||t.durum||'waiting');
 const attempted=Number(t.result?.attempted??0),assigned=Number(t.questionCount||0);
 const over=Boolean(t.dueDate&&t.dueDate<new Date().toLocaleDateString('sv-SE'));
 if(t.result&&assigned>0&&attempted>=assigned)return attempted>assigned?'Tamamlandı · Hedef üstü':'Tamamlandı';
 if(state==='cancelled')return 'İptal edildi';
 if(t.result&&attempted>0)return over?'Süresi geçti · Eksik':'Eksik · Devam ediyor';
 if(over)return 'Süresi geçti · Başlanmadı';
 return ({waiting:'Bekliyor',in_progress:'Devam ediyor',completed:'Tamamlandı',reviewed:'Değerlendirildi',pending:'Beklemede',overdue:'Süresi geçti'})[state]||'Bekliyor';
}
function assignmentProgress(t){
 if(!t.result||!(Number(t.questionCount)>0))return '';
 const count=Number(t.questionCount),actual=Number(t.result.attempted??0),diff=actual-count;
